...
if ($newPassword !== $confirmPassword) {
   ...
   // Check if OTP exists
    $query = mysqli_query($conn, "SELECT * FROM otp_tab WHERE userId = '$userId' AND otp_code = '$otp' AND expires_at > NOW()");
    ...
    $updateQuery = mysqli_query($conn, "UPDATE users_tab SET password = '$hash' WHERE userId = '$userId'") or die(mysqli_error($conn));
    if (!$updateQuery) {
        $response = [
            'response' => 500,
            'success' => false,
