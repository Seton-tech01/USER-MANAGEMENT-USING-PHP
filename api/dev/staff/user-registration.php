...
$getStaffAuthDetails = getStaffAuthDetails($conn, $userId);
if (!$getStaffAuthDetails['success']) {
    $response = [
        'response' => $getStaffAuthDetails['response'],
        'success'  => false,
        'message'  => $getStaffAuthDetails['message'],
    ];
    goto end;
}
...
$performedBy = $getStaffAuthDetails['userName'];
$performedUserId = $userId;
$userType = 'ADMIN';
$ipAddress   = $_SERVER['REMOTE_ADDR'] ?? '';
$browserName = $_SERVER['HTTP_USER_AGENT'] ?? '';
$systemName  = php_uname('s');
$logActivity = logActivities($conn, $alertId, $action, $description, $performedBy, $performedUserId, $userType, $ipAddress, $browserName, $systemName);
$response = [
    'response' => 200,
    'success' => true,
