...
$query = mysqli_query($conn, "SELECT email FROM users_tab WHERE email = '$emailAddress'") or die(mysqli_error($conn));
...
$uploadPath = $usersProfilePixPath . $passport;
...
mysqli_query($conn, "INSERT INTO users_tab( userId, titleId, firstName,  middleName, lastName, email, phoneNumber, homeAddress, genderId,  statusId, password, passport, createdTime) 
VALUES ('$userId', '$title', '$firstName','$middleName', '$lastName', '$emailAddress', '$phoneNumber', '$homeAddress', '$genderId', '1', '$inputPassword','$passport', NOW())") or die(mysqli_error($conn));
